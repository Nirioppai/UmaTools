(function () {
  ...
  function init() {
    ...
    }, { rootMargin: '-20% 0px -70% 0px' }).observe(sections[0]);
    ...
    sectionList.forEach(function (s) { observer.observe(s); });

    var resizeTimer;
    window.addEventListener('resize', function () {
      clearTimeout(resizeTimer);
      resizeTimer = setTimeout(function () { updateVisibility(nav); }, DEBOUNCE_MS);
    });

    setTimeout(function () { updateVisibility(nav); }, 100);
  }
  ...
  function buildNav(sectionList) {
    ...
    upBtn.innerHTML = '<svg viewBox="0 0 24 24"><polyline points="18 15 12 9 6 15"></polyline></svg>';
    ...
    downBtn.innerHTML = '<svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>';
    ...
    downBtn.addEventListener('click', function () {
      var target = currentIndex < sectionList.length - 1 ? currentIndex + 1 : sectionList.length - 1;
      sectionList[target].scrollIntoView({ behavior: 'smooth', block: 'start' });
      currentIndex = target;
    });
